/*
c++ -o Esercizio4TS `root-config --glibs --cflags` Esercizio4TS.cpp
*/
...
int main(int argc, char const *argv[]) {
    ...
    double ris = (exp(8) - exp(-8) - exp(3) + exp(-3))/2.;
    
    for (int i = 3; i < 10000000; i = 3*i){
        double m = 1./i;
        g_T->SetPoint(g_T->GetN(), log(m), log(Trapezio(integrando, 3, 8, i) - ris));
        g_S->SetPoint(g_T->GetN(), log(m), log(fabs(Simpson(integrando, 3, 8, i) - ris)));
    }
    ...
    g_T->GetHistogram()->GetYaxis()->SetRangeUser(-27, 8);
    g_T->Draw("AP");
    ...
    c->SetGrid();
    c->Print("Es4_EvoluzioneTS.png", "png");
