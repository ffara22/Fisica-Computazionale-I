/*
c++ -o Esercizio3TS `root-config --glibs --cflags` Esercizio3TS.cpp
*/
...
int main(int argc, char const *argv[]) {
    ...
    double ris = 5040 - 648240/exp(5);
    
    for (int i = 3; i < 10000000; i = 3*i){
        double m = 1./i;
        g_T->SetPoint(g_T->GetN(), log(m), log(Trapezio(integrando, 0, 5, i) - ris));
        g_S->SetPoint(g_T->GetN(), log(m), log(fabs(Simpson(integrando, 0, 5, i) - ris)));
    }
    ...
    c->SetGrid();
    c->Print("Es3_EvoluzioneTS.png", "png");
